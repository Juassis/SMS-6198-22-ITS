...
cols_Ecotype <- c("Meadow" = "#D46C4E", "Spruce-Alder" = "#77A515", "Spruce" = "#264D59", "Alder-zone" = "#43978D", "Pine" = "#43978D")
...
```{r}
#Shannon
p2 <- ggplot(df, aes(x=Ecotype, y=Shannon, color = Type))  +
  ...
  facet_grid(~Transect) +
  labs(hjust = 1, face= "bold", x="Ecotype ", y = "Shannon Diversity") 
p2
```
...
```{r}
#Richness
p <- ggplot(df, aes(x=Ecotype, y=Observed, color = Type))  +
  theme_pubr(border = TRUE) +
  geom_point(size=6) +
  scale_shape_manual(values=c(16,17)) +
  ...
  facet_grid(~Transect) +
  #ylim(0, 120) +
  labs(hjust = 1, face= "bold", x=" ", y = "Richness") 
p
```
...
#ggsave("/Users/juliana/Documents/NBIS/Projects/6198/Results/Fig1A_Alpha_Diversity_All_Samples.pdf", units="in", width=10, height=7.7, dpi=300)
#dev.off()
```
