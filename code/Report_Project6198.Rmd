...
The first step was check the quality of the data.
...
a
ggsave("./PLOTS/Full_Data/QC/01-fnfQC.pdf", a)
```
...
b
ggsave("./PLOTS/Full_Data/QC/02-fnRQC.pdf", b)
```
...
>Based on these profiles, I truncated the reverse reads at position 200 where the quality distribution crashes.
...
c
ggsave("./PLOTS/Full_Data/QC/03-errF.pdf", c)

```

```{r,fig.cap="Error Rate Reverse Reads.",  fig.width = 14.5, fig.height = 7.58, echo=FALSE, warning=FALSE, message=FALSE}
d <- plotErrors(errR, nominalQ = TRUE)
d
ggsave("./PLOTS/Full_Data/QC/04-errR.pdf", d)
...
histo <- data.frame(nchar(getSequences(seqtab.nochim)))
colnames(histo)[1] <- "Length"

p_hist<-ggplot(histo, aes(x=Length)) + 
  geom_histogram(color="black", fill="white")
p_hist

ggsave("./PLOTS/Full_Data/QC/05-lengh_Distribution.pdf", p_hist)
...
#head(track)
# ma <- data.frame(track)
# data_long <- melt(ma,id = "input" )# Reshaping data frame
# data_long %>%
#   ggplot(aes(variable, value, na.rm = TRUE), y=value) +
#   theme_light() +
#   geom_bar(stat="identity", position="dodge", colour="black") +
#     scale_fill_brewer(type="qual", palette=1)

ggsave("./PLOTS/Full_Data/QC/06-track_reads.pdf")
...
phylum_plot
ggsave("./PLOTS/Full_Data/Evaluation/01-Taxa_prevalence.pdf", phylum_plot)
...
**Number of unique taxa by taxonomic rank:**
...
ggsave("./PLOTS/Full_Data/Evaluation/02-plot_alpha1.pdf", plot_alpha1, width = 14.5, height = 7.58)
...
ggsave("./PLOTS/Full_Data/Evaluation/03-plot_alpha_count.pdf", alpha_count, width = 14.5, height = 7.58)
...
ggsave("./PLOTS/Full_Data/Evaluation/04-Plot_beta_distance.pdf", Beta_Replicates, width = 14.5, height = 7.58)
```
...
ggsave("./PLOTS/Full_Data/Evaluation/05-Beta_Diversity.pdf")
...
ggsave("./PLOTS/Full_Data/Evaluation/06-Beta_Diversity.pdf")
...
>In summary: there is no significative difference between A and B samples (Alpha and Beta Diversity). In this way we choose working the replicate A.
...
```{r,echo=FALSE,include=FALSE, eval = FALSE}
#df4 = data.frame(ASVs=rowSums(otu_table(P_Plots)>0), reads=sample_sums(P_Plots), sample_data(P_Plots))
df4 = data.frame(reads=sample_sums(P_Plots), sample_data(P_Plots))

df5 <- data.frame(ASVs=rowSums(otu_table(P_Plots)>0)) #[1] 3652    1
reads=data.frame(sample_sums(P_Plots)) #[1]  45 1
ppp = data.frame(sample_data(P_Plots)) #45 10

ggplot(df4, aes(x=reads)) + geom_histogram(bins=50, color='black', fill='grey') + 
...
ggsave("./PLOTS/Full_Data/Evaluation/07-Rarefaction.pdf")

out = rarecurve(otu_table(psdata), col=nice_colors, step=100 , lwd=2, ylab="ASVs", label=F,
...
```{r,fig.cap="Rarefaction.",  fig.width = 14.5, fig.height = 7.58, echo=FALSE, warning=FALSE, message=FALSE}
ggsave("./PLOTS/Full_Data/Evaluation/08-Rarefaction.pdf")
...
names(out) = rownames(otu_table(psdata))
...
xy = data.frame(xy, 
  sample_data(psdata)[match(xy$SampleID, rownames(sample_data(psdata))), ])
...
##
ggsave("./PLOTS/Main_Analysis/00_Alpha_Diversity/01-plot_alpha.pdf", width = 14.5, height = 7.58)
options(ggrepel.max.overlaps = Inf)
p <- df2 %>% 
gather(key = metric, value = value, c("Observed", "Shannon")) %>%
...
Method = wilcox.test with p.adjust.method BH
...
In this step I performed the Alpha diversity using the mean of the 3 samples per Ecotype.
...
Non-metric Multi-dimensional Scaling - NMDS, is a way to condense information from multidimensional data, into a 2D representation or ordination. In this ordination, the closer two points are, the more similar the corresponding samples are with respect to the variables that went into making the NMDS plot.

In this study, NMDS shows to be the best method for ordination analysis. Our data has a skewed distribution with a long tail.
...
```{r,fig.cap="Beta Diversity - A.",  fig.width = 14.5, fig.height = 7.58, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r,fig.cap="Beta Diversity - B.",  fig.width = 14.5, fig.height = 7.58, echo=FALSE, warning=FALSE, message=FALSE}
...
**Beta Diversity by Transect**

```{r,fig.cap="Beta Diversity - C.",  fig.width = 14.5, fig.height = 7.58, echo=FALSE, warning=FALSE, message=FALSE}
...
**Beta Diversity by Ecotype**

```{r,fig.cap="Beta Diversity - D.",  fig.width = 14.5, fig.height = 7.58, echo=FALSE, warning=FALSE, message=FALSE}
...
To normalize the data, I used the qPCR information:
...
> Step 3: _Correct_ the data: Multiple by the qPCR information\
...
```{r,fig.cap="AlphaDiversity Main Analysis.",  fig.width = 18.5, fig.height = 17.58, echo=FALSE, warning=FALSE, message=FALSE}
...
The same filters has been performed in this step. The difference here is the way I'm showing the result.
...
**Relative abundance of the top 20 Genus**

Again, I applied the same data transformation.
...
```{r,fig.cap="AlphaDiversity Main Analysis.",  fig.width = 14.5, fig.height = 14.58, echo=FALSE, warning=FALSE, message=FALSE}
