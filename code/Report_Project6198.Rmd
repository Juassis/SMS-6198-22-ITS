...
#
batch1_media_melt <- melt(batch1_taxonomy_otu_metadata, id.vars=c("Row.names", "Kingdom","Phylum", "Class", "Order", "Family", "Genus","Species", "ASV_id.1","ASV", "taxa_name"))

colnames(batch1_media_melt) <- c("Row.names", "Kingdom","Phylum", "Class", "Order", "Family", "Genus","Species","ASV_id.1","ASV", "taxa_name","sample", "RA")
#
...
#Using raref 
# psdata
# write_phyloseq(P_Plots, type = "METADATA")

# P_Plots@sam_data$CopyNumber <- as.numeric (c("56547216,06","1958795290","1610243098","176573431,2","10309433,02","4102423565","656843900,4","693875330,7","115008831,8","3378601403","1554745882","3631465937","774323956,3","87565573,72","377544210,5","3189735171","297379382,4","66300059,27","653127690,5","7015954105","90486793,16","127655843,7","3687956899","3857200314","3295133438","1124769464","80251070,77","5825489676","2509789932","370848302,5","5321537786","3487721616","2811105415","7290249897","111414313,2","7391533559","4369972598","7528113879","1231526770","3176809071","10563935153","912523229,2","6437853632","2832182794","1592097562"))
...
#MetaData
#adding metadata
# genefilter_tax2@sam_data$CopyNumber <- as.numeric (c("56547216,06","1958795290","1610243098","176573431,2","10309433,02","4102423565","656843900,4","693875330,7","115008831,8","3378601403","1554745882","3631465937","774323956,3","87565573,72","377544210,5","3189735171","297379382,4","66300059,27","653127690,5","7015954105","90486793,16","127655843,7","3687956899","3857200314","3295133438","1124769464","80251070,77","5825489676","2509789932","370848302,5","5321537786","3487721616","2811105415","7290249897","111414313,2","7391533559","4369972598","7528113879","1231526770","3176809071","10563935153","912523229,2","6437853632","2832182794","1592097562"))

metaR <- meta(genefilter_tax2)

# Reorder Transect
metaR$Transect = factor(metaR$Transect, levels = c("1", "2","3")) #D46C4E
metaR_Transect <- c("#264D59","#77A515","#D46C4E") #D46C4E #43978D
...
SampleOrder = order(metaR$Transect, metaR$Ecotype)
...
abs_plot <- data.frame(abundances(metaR_Filter_composi.filt.abs))
...
################3
# meta_ra <- data.frame(meta(metaR_Filter_composi.filt.abs))
# Label_ra <- rownames(meta_ra)
# sample_data(metaR_Filter_composi.filt.abs)$Label <- Label_ra
# meta_ra2 <- data.frame(meta(metaR_Filter_composi.filt.abs))
# 
# taxonomy_ra <- data.frame(tax_table(metaR_Filter_composi.filt.abs))
# abs_otu <- data.frame(abundances(metaR_Filter_composi.filt.abs))
# 
# 
# #
# taxonomy_otu_metadata <- merge(taxonomy_ra, abs_otu, by=0, all=TRUE)
# #
# media_melt <- melt(taxonomy_otu_metadata, id.vars=c("Row.names", "Kingdom","Phylum", "Class", "Order", "Family", "Genus","Species", "ASV_id","ASV", "taxa_name"))
# colnames(media_melt) <- c("Row.names", "Kingdom","Phylum", "Class", "Order", "Family", "Genus","Species","ASV_id","ASV", "taxa_name","sample", "RA")
# #
# taxa_all_metadata2 =  merge(media_melt, meta, by.x = "sample", by.y = "Label")
...
# meta_taxonomy_otu_compositional_copy <- data.frame(sample_data(WTOE_Filter_composi.filt.ab))
# meta_m <- rownames(meta_taxonomy_otu_compositional_copy)
# colnames(abs_plot) <- meta_m
...
#plot_Log10_max[] = lapply(plot_Log10_max, sub, pattern = "-Inf", replacement = "", fixed = TRUE)
...
#data_subset_norm <- t(apply(abs_plot, 1, cal_z_score))
...
pheatmap(plot_Log10_max[, SampleOrder],
         ...
         annotation_colors = metaR_AnnColour, annotation_col = meta.factors, 
         #annotation_colors = OE_WT_AnnColour, 
         show_colnames = FALSE,
         color = colorRampPalette(c("white", colsHeat))(50),
         border_color = "#f8edeb",
         display_numbers = FALSE)

#########colsH <- colorRampPalette(rev(brewer.pal(5,"RdBu")))(20)
...
#ampvis2_obj <- phyloseq_to_ampvis2(pseq_plot_heat)
