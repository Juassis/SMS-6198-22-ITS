...
#Subset
top <- names(sort(taxa_sums(phylo.relative), decreasing=TRUE))[1:100]
...
speciesList = unique(tax_table(ps.top)[,"taxa_name"])
speciesPalette = getPalette(length(speciesList))
names(speciesPalette) = speciesList
#Change for Species, Genus
ggplot(data=Batch1_taxa_all_metadata2, aes(x=Ecotype, group = taxa_name)) +
  geom_bar(aes(y=RA, fill = taxa_name),stat="identity", alpha=.7) + 
  theme_pubr(border = TRUE) +
